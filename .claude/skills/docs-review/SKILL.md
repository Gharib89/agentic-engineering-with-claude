...
name: docs-review
description: >-
  Re-review shipped Chapters claim-vs-source; confirmed findings land
...
review tickets (titled `docs-review: <chapter>`) and take the Chapter
longest unreviewed; when the search is inconclusive, ask which Chapter to
review.
...
source (WebFetch, or an index query when the source is an index), and
verdicts the claim: *holds*, or *candidate finding* with the evidence.
External links are not CI-validated, so source rot is exactly what the
reviewer hunts; a 404 can be bot-UA gating, not rot — retry with a
browser `User-Agent` before recording a dead link (`aihero.dev` is a
known case).
...
Lane, titled `docs-review: <chapter>` so step 1's stalest-chapter search
finds it. When several Chapters yield only very small findings, merge
