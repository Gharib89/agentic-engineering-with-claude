...
Cheap implementation makes the plan the expensive part: a vague one ships working code that solves the wrong problem by lunchtime. So stop writing it alone — let the agent interview you until every open question is a decision you made, then cut the answers into a blueprint and its work orders.
...
Every question a planning session raises falls on one side of a line. A question of fact — what the platform supports, what the standard says, what the existing system does — has an answer out in the world, so the agent fetches it instead of asking you. A question of decision — scope, trade-offs, acceptable risk — has none; someone has to make one. The interview ends when every open question is a decision, made and written down.
...
So do not brief the agent — let it grill you; an interview skill that pushes back relentlessly, like the [grilling skill](https://www.aihero.dev/skills-grill-with-docs), holds the line.
...
The interview and the research keep turning up concepts that need names. Name each one where it appears: define the term once in the shared phrasebook, list beside it the synonyms the project refuses, and write any system-shaping choice into the decision diary then and there. A [domain-modeling skill](https://www.aihero.dev/skills-domain-modeling) makes both moves routine; there is no modeling phase, only the rule. [Chapter 2](02-project-setup.md) scaffolded both files empty; planning is when they fill, and agents repeat back the vocabulary you give them.

## 4. The blueprint is a parent, not a work item

The decisions, the findings, and the settled vocabulary converge in one blueprint: the problem, the shape of the solution, the decisions made — with the options rejected — and what is out of scope. It is the relay baton of the effort: the next session reads the decisions, never the chat that produced them, and a [to-spec skill](https://www.aihero.dev/skills-to-spec) drives the dialogue into that document. But nothing builds "the spec" itself, so a blueprint never enters a queue.

## 5. Every work order is a tracer bullet

What enters the queue are work orders cut from the blueprint — a [to-tickets skill](https://www.aihero.dev/skills-to-tickets) does the cutting — and each is a **Tracer Bullet**: a thin slice through every layer, ending in something a user or a check can see.

![One layer at a time fills a whole row of the stack and nothing runs end to end, so no check can see it yet; a tracer bullet fills one thin column across every layer, runs end to end, and a check can see it](../assets/diagrams/tracer-bullet-slice.svg#only-light)
![One layer at a time fills a whole row of the stack and nothing runs end to end, so no check can see it yet; a tracer bullet fills one thin column across every layer, runs end to end, and a check can see it](../assets/diagrams/tracer-bullet-slice.dark.svg#only-dark)

*A layer built in isolation leaves nothing to check; a thin slice through every layer ends in evidence a check can see.*

Each carries its goal, the proof photo that shows it done, and pointers to the context it needs. Order lives in blocking edges, not in anyone's head, so "what can start right now?" is a query against the job board.

!!! example "this repo — one blueprint, eight tracer bullets"
    The Guide's spec is one parent issue holding the problem statement and
    every decision. Its tickets are sub-issues, one per Chapter, each
    shipping one page end to end. This page's ticket blocked on the Docs
    Lane ticket: until the lane existed, no query would surface it as
    ready.

## 6. Too foggy for a blueprint? Map the fog first

Some efforts resist a single interview: the unknowns interlock, each answer moving the others, and forcing a blueprint there only launders guesses. Map the effort instead, with a [wayfinder skill](https://www.aihero.dev/skills-wayfinder): a **Wayfinder Map** holds notes, the decisions made so far, and the fog of open questions still ahead.
...
Every picture above, anchored to its real term:
