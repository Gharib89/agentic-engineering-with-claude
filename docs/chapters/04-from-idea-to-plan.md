...
![A wayfinder map: one map record holding notes, decisions-so-far, and the fog; typed child tickets — research, prototype, grilling, task — hang off it with blocking edges, and resolved children write their answers back to the map](../assets/diagrams/wayfinder-map.svg)
