...
!!! example "this repo — tickets sized to the smart zone"
    This repo — the Guide's own source, one of its Reference Repos — ships
    every Guide ticket through a docs-ship run that handles exactly one
    ticket, so each ticket gets a fresh session and the sharpest part of
